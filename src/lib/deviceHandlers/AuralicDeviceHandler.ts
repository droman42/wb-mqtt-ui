...
export class AuralicDeviceHandler implements DeviceClassHandler {
  ...
  private processAllGroupActions(groups: DeviceGroups): ProcessedAction[] {
    if (!groups.groups) {
      console.log('⚠️  [AuralicDevice] No groups found in device groups');
      return [];
    }
    ...
    for (const group of filteredGroups.groups!) {
      if (!group.actions) {
        console.log(`⚠️  [AuralicDevice] No actions found in group: ${group.group_name}`);
        continue;
      }
    ...
    console.log(`📊 [AuralicDevice] Processed ${allActions.length} total actions from ${filteredGroups.groups!.length} groups`);
    return allActions;
  }
  ...
  private filterExcludedGroups(groups: DeviceGroups): DeviceGroups {
    ...
    const filteredGroups = groups.groups.filter(group => {
      ...
      if (shouldExclude) {
        console.log(`🚫 [AuralicDevice] Excluding group '${group.group_name}' from UI (${group.actions.length} actions)`);
      }
    ...
    if (filteredGroups.length !== originalCount) {
      console.log(`🔍 [AuralicDevice] Filtered groups: ${originalCount} → ${filteredGroups.length} (excluded ${originalCount - filteredGroups.length} groups)`);
    }
