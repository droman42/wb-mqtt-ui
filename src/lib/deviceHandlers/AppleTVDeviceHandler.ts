...
export class AppleTVDeviceHandler implements DeviceClassHandler {
  ...
  private processAllGroupActions(groups: DeviceGroups): ProcessedAction[] {
    ...
    console.log(`📊 [AppleTV] Processed ${allActions.length} total actions from ${filteredGroups.groups!.length} groups`);
    return allActions;
  }
  ...
  private filterExcludedGroups(groups: DeviceGroups): DeviceGroups {
    ...
    const filteredGroups = groups.groups.filter(group => {
      ...
      if (shouldExclude) {
        console.log(`🚫 [AppleTV] Excluding group '${group.group_name}' from UI (${group.actions.length} actions)`);
      }
    ...
    if (filteredGroups.length !== originalCount) {
      console.log(`🔍 [AppleTV] Filtered groups: ${originalCount} → ${filteredGroups.length} (excluded ${originalCount - filteredGroups.length} groups)`);
    }
