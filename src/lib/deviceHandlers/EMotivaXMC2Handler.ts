...
export class EMotivaXMC2Handler implements DeviceClassHandler {
  ...
  private processAllGroupActions(groups: DeviceGroups): ProcessedAction[] {
    ...
    console.log(`📊 [EMotivaXMC2] Processed ${allActions.length} total actions from ${filteredGroups.groups!.length} groups`);
    return allActions;
  }
  ...
  private filterExcludedGroups(groups: DeviceGroups): DeviceGroups {
    ...
    const filteredGroups = groups.groups.filter(group => {
      ...
      if (shouldExclude) {
        console.log(`🚫 [EMotivaXMC2] Excluding group '${group.group_name}' from UI (${group.actions.length} actions)`);
      }
    ...
    if (filteredGroups.length !== originalCount) {
      console.log(`🔍 [EMotivaXMC2] Filtered groups: ${originalCount} → ${filteredGroups.length} (excluded ${originalCount - filteredGroups.length} groups)`);
    }
